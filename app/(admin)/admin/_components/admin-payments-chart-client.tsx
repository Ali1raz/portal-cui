...
import type { AdminPaymentsChartPoint } from "@/app/data/admin/get-admin-dashboard";
...
import { formatFineAmount } from "@/lib/utils/fine-calculation";
...
type AdminPaymentsChartClientProps = {
  data: AdminPaymentsChartPoint[];
};

export function AdminPaymentsChartClient({
  data,
}: AdminPaymentsChartClientProps) {
    ...
    <Card className="pt-0">
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
          <CardTitle>Payments - {chartRangeLabel(selectedRange)}</CardTitle>
          <CardDescription>
            Showing {filteredData.length} days — total{" "}
            {formatFineAmount(totalAmount)}
          </CardDescription>
        </div>
        ...
        >
          <BarChart data={filteredData}>
            ...
            <YAxis
              ...
              allowDecimals={false}
              domain={[0, "auto"]}
              tickFormatter={(value) => formatFineAmount(Number(value))}
            />
            <ChartTooltip
              cursor={false}
              content={
                <ChartTooltipContent
                  ...
                  formatter={(val) => formatFineAmount(Number(val))}
                  indicator="dot"
                />
              }
            />
...
export default AdminPaymentsChartClient;
