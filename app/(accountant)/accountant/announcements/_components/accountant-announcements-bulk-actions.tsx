...
import {
  accountantBulkDeleteAnnouncements,
  accountantBulkUpdateAnnouncementStatus,
} from "../actions";
...
export function AccountantAnnouncementsBulkActions({
  selectedIds,
  announcements,
  onSuccess,
}: {
  ...
  function handleBulkDelete() {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(
        accountantBulkDeleteAnnouncements(selectedIds)
      );
  ...
  function handleStatusUpdate(newStatus: AnnouncementStatus) {
    ...
    startTransition(async () => {
      const { data: result, error } = await tryCatch(
        accountantBulkUpdateAnnouncementStatus(selectedIds, newStatus)
      );
