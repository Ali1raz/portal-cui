...
export const createInstallmentSplitRequestSchema = (remainingAmount: number) =>
  z
    .object({
      splitAmount: z
        .number()
    ...
    .refine((data) => data.splitAmount < remainingAmount, {
      path: ["splitAmount"],
      message: `Split amount must be less than available amount (${remainingAmount})`,
    })
...
export type CreateInstallmentSplitRequestSchemaType = z.infer<
  ReturnType<typeof createInstallmentSplitRequestSchema>
>;
