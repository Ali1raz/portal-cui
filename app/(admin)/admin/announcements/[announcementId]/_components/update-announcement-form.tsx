...
import { AdminGetAnnouncementForUpdateType } from "@/app/data/admin/get-announcement-for-update";
import { adminUpdateAnnouncement } from "../../actions";

/// Form for admins to update existing announcements with targeting options.
export function UpdateAnnouncementForm({
  announcement,
}: {
  /// Existing announcement data to pre-populate form.
  announcement: AdminGetAnnouncementForUpdateType;
}) {
  ...
  function onSubmit(values: AdminAnnouncementSchemaType) {
    ...
    startTransition(async () => {
      const { data: result, error } = await tryCatch(
        adminUpdateAnnouncement(announcement.id, values)
      );
      if (error) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-4 rounded-lg border p-4">
          <h3 className="font-medium text-sm">Audience Targeting (Optional)</h3>
          <p className="text-xs text-muted-foreground">
            Leave field unselected to target all users. Select specific
            department to limit the announcement audience.
          </p>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel htmlFor="target-department">
                  Target Department
                </FormLabel>
                <Select
                  onValueChange={(value) =>
